...
import { Mail, MessageSquare, Share2, PenTool, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="pb-24 pt-32 relative min-h-screen bg-[#06030a]">
      <div className="absolute top-0 inset-x-0 h-[700px] md:h-[800px] z-0 pointer-events-none">
        <div className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-70" style={{ backgroundImage: 'url(/images/img_042.webp)' }}></div>
      ...
      <div className="container relative z-10">
        <div className="max-w-4xl mx-auto text-center mb-32 pt-16">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}>
            <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-md">
              ...
              <span className="text-white/80 text-xs font-bold tracking-[0.2em] uppercase">Audience Distribution</span>
            </div>
            <h1 className="text-6xl md:text-7xl lg:text-[6rem] font-black text-white uppercase tracking-tighter leading-[0.9] font-heading mb-8">SOCIAL MEDIA<br/>MARKETING</h1>
            <p className="text-white/60 text-xl md:text-2xl font-light max-w-2xl mx-auto tracking-wide leading-relaxed">Build loyal communities and dominate feed algorithms with continuous, high-value organic content strategies.</p>
          </motion.div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-32">
          <BentoCard 
            ...
            desc="End-to-end management of your social feed. We produce, schedule, and engage, ensuring your brand stays top of mind."
            icon={<PenTool size={20} strokeWidth={2} />}
            features={["Instagram & TikTok", "Grid Planning", "Community Management"]}
            linkTo="/services/social-media/content-creation"
          />
          <BentoCard delay={0.2} title="Email Marketing" desc="Transform followers into customers through deeply personalized, high-converting automated email sequences." icon={<Mail size={20} strokeWidth={2} />} features={["Newsletter Campaigns", "Abandoned Cart Flows", "List Segmentation"]} linkTo="/services/social-media/email-marketing" />
          <BentoCard 
            delay={0.3}
            title="WhatsApp Marketing"
            desc="Reach customers where they chat. Direct, high-open-rate broadcasts and automated customer service funnels."
            icon={<MessageSquare size={20} strokeWidth={2} />}
            features={["Automated Flows", "Broadcasts", "Customer Support Bots"]}
            linkTo="/services/social-media/whatsapp-marketing"
          />
          <BentoCard 
            delay={0.4}
            title="LinkedIn Marketing"
            desc="B2B dominance. We optimize profiles and create thought-leadership content that generates high-ticket leads."
            icon={<Share2 size={20} strokeWidth={2} />}
            features={["Executive Branding", "Lead Generation", "Company Pages"]}
            linkTo="/services/social-media/linkedin-marketing"
          />
        </div>
        <FadeIn>
          <div className="glass rounded-[40px] p-12 md:p-20 text-center border border-white/5 relative overflow-hidden bg-white/[0.01]">
            <h2 className="text-4xl md:text-5xl font-black text-white mb-6 uppercase font-heading tracking-tight">Ready to go viral?</h2>
            <p className="text-white/50 mb-10 max-w-xl mx-auto text-xl font-light">Let's build an audience that actually converts.</p>
            <MagneticButton>
              <Link to="/contact" className="btn btn-primary px-10 py-5 text-lg font-bold tracking-widest uppercase">Start Campaign <ArrowRight className="ml-2 inline-block" size={20}/></Link>
            </MagneticButton>
          </div>
        </FadeIn>
      </div>
