...
import { Scissors, Film, Image as ImageIcon, MonitorPlay, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="pb-24 pt-32 relative min-h-screen bg-[#06030a]">
      <div className="absolute top-0 inset-x-0 h-[700px] md:h-[800px] z-0 pointer-events-none">
        <div className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-70" style={{ backgroundImage: 'url(/images/img_039.webp)' }}></div>
      ...
      <div className="container relative z-10">
        <div className="max-w-4xl mx-auto text-center mb-32 pt-16">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}>
            <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-md">
              ...
              <span className="text-white/80 text-xs font-bold tracking-[0.2em] uppercase">Post-Production</span>
            </div>
            <h1 className="text-6xl md:text-7xl lg:text-[6rem] font-black text-white uppercase tracking-tighter leading-[0.9] font-heading mb-8">CREATIVE<br/>EDITING</h1>
            <p className="text-white/60 text-xl md:text-2xl font-light max-w-2xl mx-auto tracking-wide leading-relaxed">Masterful post-production that turns raw footage into thumb-stopping stories and cinematic masterpieces.</p>
          </motion.div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-32">
          <BentoCard 
            delay={0.1}
            title="Reels & Shorts"
            desc="High-retention editing engineered specifically for TikTok and Instagram algorithms to maximize watch time."
            icon={<Scissors size={20} strokeWidth={2} />}
            features={["Dynamic Captions", "Pacing & Hooks", "Audio Syncing"]}
            linkTo="/services/editing/reels-shorts"
          />
          <BentoCard 
            delay={0.2}
            title="Cinematic Vlog Editing"
            desc="Long-form storytelling that keeps YouTube audiences engaged from the intro hook to the end screen."
            icon={<Film size={20} strokeWidth={2} />}
            features={["Narrative Flow", "Color Grading", "Sound Design"]}
            linkTo="/services/editing/cinematic-vlog"
          />
          <BentoCard delay={0.3} title="Wedding Films" desc="Emotional, beautifully paced edits that capture the magic of the day with perfect musical synchronization." icon={<Scissors size={20} strokeWidth={2} />} features={["Highlight Reels", "Full Ceremony Edits", "Audio Mixing"]} linkTo="/services/editing/wedding-films" />
          <BentoCard 
            delay={0.4}
            title="Photo Retouching"
            desc="Professional grade color correction and skin retouching for fashion, real estate, and product photography."
            icon={<ImageIcon size={20} strokeWidth={2} />}
            features={["High-End Skin Retouching", "Compositing", "Batch Color Correction"]}
            linkTo="/services/editing/photo-retouching"
          />
        </div>
        <FadeIn>
          <div className="glass rounded-[40px] p-12 md:p-20 text-center border border-white/5 relative overflow-hidden bg-white/[0.01]">
            <h2 className="text-4xl md:text-5xl font-black text-white mb-6 uppercase font-heading tracking-tight">Ready to cut?</h2>
            <p className="text-white/50 mb-10 max-w-xl mx-auto text-xl font-light">Send us your raw files and we'll send back a masterpiece.</p>
            <MagneticButton>
              <Link to="/contact" className="btn btn-primary px-10 py-5 text-lg font-bold tracking-widest uppercase">Start Project <ArrowRight className="ml-2 inline-block" size={20}/></Link>
            </MagneticButton>
          </div>
        </FadeIn>
      </div>
